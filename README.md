...
4.  Создайте и отредактируйте файл с переменными окружения:
    ...
    **Параметры режима ASN (по провайдерам) — РЕКОМЕНДУЕТСЯ:**
    ...
    - `ASN_DATABASE_PATH`: Путь к файлу GeoLite2-ASN.mmdb. По умолчанию `/app/data/GeoLite2-ASN.mmdb`.
    ...
    **Дополнительный шаг для ASN режима:**

    Если вы выбрали ASN режим (`DETECT_BY_ASN=true`), необходимо скачать базу данных GeoLite2 ASN:

    ```bash
    # Создайте директорию для базы
    mkdir -p observer_conf/data

    # Следуйте инструкциям в файле ASN_DATABASE_SETUP.md
    ...
    # === ВАРИАНТ 2: MaxMind (требует регистрацию, максимальная актуальность) ===
    # 1. Зарегистрируйтесь: https://www.maxmind.com/en/geolite2/signup
    # 2. Получите лицензионный ключ в аккаунте
    # 3. Скачайте базу (замените YOUR_LICENSE_KEY):
    cd observer_conf/data
    ...
    **Примечание:** База обновляется MaxMind каждый вторник. Настройте автоматическое обновление через cron (см. [ASN_DATABASE_SETUP.md](ASN_DATABASE_SETUP.md)).
